...
BM25의 길이 정규화(b)나 포화 항(k1)이 없으므로 BM25라고 부르지 않습니다.
파이프라인이 실제로 통하는지 확인하려는 것이지 검색 성능을 주장하는 코드가 아닙니다.

검색 품질을 실제로 측정하는 계획은 [`docs/RETRIEVAL_EXPERIMENT_PLAN.md`](docs/RETRIEVAL_EXPERIMENT_PLAN.md)에
있습니다 — 평가셋 설계, 정답을 원문 좌표에 거는 방식, 지표, 검색기 비교 순서를 실행 전에 고정해 둔 문서입니다.
